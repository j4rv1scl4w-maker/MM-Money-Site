...
function renderBody(body: string) {
  return body.split('\n\n').map((para, i) => {
    const html = para
      ...
      .replace(/\*(.+?)\*/g, '<em>$1</em>');
    return (
      <p key={i} style={{ marginBottom:20 }} dangerouslySetInnerHTML={{ __html: html }} />
    );
  });
}
  ...
  return (
    <article style={{ maxWidth:740, margin:'0 auto', padding:'44px 24px 80px' }}>
      ...
      <div style={{ font:'600 12px/1 Hanken Grotesk,sans-serif', color:'var(--ink2)', marginBottom:20, display:'flex', gap:8 }}>
        <Link href="/articoli" style={{ color:'var(--ink2)', textDecoration:'none' }}>Articoli</Link>
        <span>/</span>
      ...
      <div style={{ font:'600 12.5px/1 Hanken Grotesk,sans-serif', color:'var(--ink2)', marginBottom:28 }}>
        MM·Money · {article.read} · aggiornato 2026
      </div>
      ...
      <div style={{ marginTop:32 }}>
        <Link href="/articoli" style={{ font:'600 13px/1 Hanken Grotesk,sans-serif', color:'var(--gold)', textDecoration:'none' }}>← Tutti gli articoli</Link>
      </div>
    </article>
  );
}
